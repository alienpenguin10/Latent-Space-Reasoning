...
def apply_cross_path_interaction_batch(
    ...
    if current_embeds.dim() != 2:
        raise ValueError(f"[M3PO] current_embeds must be 2D, got shape {current_embeds.shape}")
    if current_embeds.shape[0] % num_generations != 0:
        raise ValueError(f"[M3PO] current_embeds.shape[0]={current_embeds.shape[0]} not divisible by num_generations={num_generations}")
    
    batch_size = current_embeds.shape[0] // num_generations
    hidden_dim = current_embeds.shape[1]
    
    if debug:
        print(f"[M3PO MODULE BATCH] batch_size={batch_size}, num_generations={num_generations}, hidden_dim={hidden_dim}, device={current_embeds.device}")
...
def apply_cross_path_interaction(
    ...
    if debug:
        print(f"[M3PO MODULE INTERACT] N={N}, hidden_dim={hidden_dim}, device={device}, dtype={dtype}")
